...
- 0 ——未进行源验证。
- 1 ——开启严格的反向路由校验。对每个进来的数据包，校验其反向路由是否是最佳路由。如果反向路由不是最佳路由，则直接丢弃该数据包（[RFC3704](https://datatracker.ietf.org/doc/html/rfc3704) 所定义的严格模式）。
- 2 ——开启松散的反向路由校验。对每个进来的数据包，校验其源地址是否可达，即反向路由是否能通（通过任意网口），如果反向路径不通，则直接丢弃该数据包（[RFC3704](https://datatracker.ietf.org/doc/html/rfc3704) 所定义的松散模式）。
...
只要网卡rp_filter 和net.ipv4.conf.all.rp_filter 有一个是1，则网卡的rp_filter 就会被启动。
